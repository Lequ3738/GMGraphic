...
{
	// Assembles && creates pixel shader.  Returns handle.
	string       str = src_asm;
	string       err;
	dword        shader;
	LPD3DXBUFFER psc;
...
{
	// Set active pixel shader, || -1 to disable.
	if (shader < 0)
	{
...
{
	// Set active pixel shader && configuration.
	bool a, b;
...
{
	// Set current vertex shader || -1 for none.  GM's normal drawing functions are not affected.
	if (shader < 0)
	{
		vbuff_usevs = false;
...
{
	// Creates new pixel shader configuration && returns its index.
	ps_conf conf;
